...
export default function EditPortfolio() {
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            if (!res.ok) { alert("Not found"); router.replace("/dashboard/portfolio"); return; }
            ...
            editor?.commands.setContent(data.contentHtml || "<p></p>");
            setLoading(false);
        })();
    }, [id, mounted, editor, router]);
    ...
    const save = async () => {
        ...
        setSaving(false);
        if (res.ok) { setSavedAt(new Date()); router.refresh(); }
        else alert("Failed to save");
    };

    if (!mounted) return <div className="p-6">Loading editor…</div>;
            ...
            <div className="-mx-5 border-b border-black/5 bg-white px-5 py-3">
                <div className="mx-auto flex max-w-6xl items-center justify-between">
                    <div className="min-w-0">
                        ...
                        <div className="truncate text-base font-medium text-[#0f6f70]">{title || "Untitled"}</div>
                    </div>
                    <div className="flex items-center gap-2">
                        ...
                        {slug && (
                            <a href={`/portfolio/${slug}`} target="_blank" className="rounded-md border px-3 py-1.5 text-sm text-[#0f6f70] hover:bg-[#f0fbfb]">
                                View
                            </a>
                        )}
                        <button
            ...
            <div className="grid gap-6 lg:grid-cols-[1fr_320px]">
                ...
                <section className="overflow-hidden rounded-2xl border border-black/5 bg-white shadow-sm h-[75vh] flex flex-col">
                    ...
                    <div className="border-b border-black/5 bg-white px-4 py-3 shrink-0">
                        <div className="flex flex-wrap items-center gap-2">
                            ...
                            <div className="flex items-center gap-1">
                                ...
                                <select className="rounded-md border px-2 py-1.5 text-sm" value={activeHeading} onChange={(e) => setHeading(Number(e.target.value))} title="Heading">
                    ...
                    <div className="flex-1 px-4 py-4 overflow-y-auto">
                        {editor ? <EditorContent editor={editor} /> : <div className="text-sm text-gray-500">Loading editor…</div>}
                    </div>
                </section>
                ...
                <aside className="space-y-4">
                    <div className="rounded-2xl border border-black/5 bg-white p-4 shadow-sm">
                        ...
                        <div className="mt-4 text-xs text-gray-500">Slug</div>
                        <div className="truncate text-sm text-gray-700">{slug || "—"}</div>

                        {slug && (
                            <a href={`/portfolio/${slug}`} target="_blank" className="mt-3 inline-block rounded-md border px-3 py-1.5 text-sm text-[#0f6f70] hover:bg-[#f0fbfb]">
                                Preview
                            </a>
                        )}
                    </div>
